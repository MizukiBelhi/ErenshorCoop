...
namespace ErenshorCoop.Steam
{
	public static class Networking
	{
		...
		//private static List<HSteamNetConnection> _connectedClients = new List<HSteamNetConnection>();
		...
		public static void StartHost(int port, bool directConnect=false)
		{
			...
			{
				Logging.Log("Steam Sockets host already started.");
				return;
			}
			...
			Logging.Log($"Steam Sockets host started. Listening on socket: {_listenSocket.m_HSteamListenSocket}:{port}.");
		...
		public static void Cleanup()
		{
			...
			if (_listenSocket != HSteamListenSocket.Invalid)
			{
				...
				Logging.Log("Steam Sockets listen socket closed.");
			}
		...
		private static void ProcessIncomingMessage(SteamNetworkingMessage_t msg)
		{
			...
			{
				Logging.LogError($"Error processing incoming Steam Sockets message: {ex.Message}");
			}
		}
		...
		private static void OnSteamNetConnectionStatusChanged(SteamNetConnectionStatusChangedCallback_t pCallback)
		{
			...
			switch (info.m_eState)
			{
				...
				case ESteamNetworkingConnectionState.k_ESteamNetworkingConnectionState_Connecting:
					if (_listenSocket != HSteamListenSocket.Invalid)
					{
						if (Lobby.isInLobby && Lobby.GetLobbyMembersSteamID().Contains(remoteSteamID))
						{
							...
							if (res == EResult.k_EResultOK)
							{
								...
								Logging.Log($"[SteamSockets Host] Accepted new connection from {remoteSteamID}.");

								ServerConnectionManager.Instance?.OnPeerConnected(remoteSteamID);
							}
							else
							{
								Logging.LogError($"[SteamSockets Host] Failed to accept connection from {remoteSteamID}. Result: {res}");
				...
				case ESteamNetworkingConnectionState.k_ESteamNetworkingConnectionState_Connected:
					Logging.Log($"[SteamSockets] Connected: {debugInfo}");
					...
					if (_connection != HSteamNetConnection.Invalid)
					{
						...
						Logging.Log($"[SteamSockets Client] Successfully connected to host: {remoteSteamID}.");
						}
					}
					if (_listenSocket != HSteamListenSocket.Invalid)
					{
						...
						Logging.Log($"[SteamSockets Host] CLIENT: {remoteSteamID} - {Lobby.playerSteamID}.");
					}
					break;

				case ESteamNetworkingConnectionState.k_ESteamNetworkingConnectionState_ClosedByPeer:
				case ESteamNetworkingConnectionState.k_ESteamNetworkingConnectionState_ProblemDetectedLocally:
					Logging.Log($"[SteamSockets] Disconnected: {debugInfo}. Reason: {info.m_szEndDebug}, OldState: {oldState}, RemoteState: {info.m_eState}");
					...
					if (oldState == ESteamNetworkingConnectionState.k_ESteamNetworkingConnectionState_Connected)
					{
						if (_listenSocket != HSteamListenSocket.Invalid)
						{
							...
							Logging.Log($"[SteamSockets Host] Client {remoteSteamID} disconnected.");

							ServerConnectionManager.Instance?.OnPeerDisconnected(remoteSteamID, info.m_szEndDebug);
							UI.Connect._feedbackText.text = $"{info.m_eState}";
						...
						else if (hConn == _connection)
						{
							...
							Logging.Log($"[SteamSockets Client] Disconnected from host: {remoteSteamID}.");
		...
		public static bool CollectPlayerData()
		{
			...
			lastPlayerData.Add(new() { name = "[HOST] "+GameData.CurrentCharacterSlot.CharName, ping = 0, zone = SceneManager.GetActiveScene().name });
			...
			foreach (var usr in _steamIdToConnection)
			{
				...
				lastPlayerData.Add(new() { name = ent.entityName, ping = _status.m_nPing, zone = ent.zone });
			}


			lastPlayerData.Add(new() { name = "Scrubby", ping = UnityEngine.Random.Range(200,500), zone = SceneManager.GetActiveScene().name });
			lastPlayerData.Add(new() { name = "Cyndara", ping = UnityEngine.Random.Range(35, 65), zone = SceneManager.GetActiveScene().name });
			lastPlayerData.Add(new() { name = "Behox", ping = UnityEngine.Random.Range(101, 200), zone = SceneManager.GetActiveScene().name });
			lastPlayerData.Add(new() { name = "Turk", ping = UnityEngine.Random.Range(1, 5), zone = SceneManager.GetActiveScene().name });
			lastPlayerData.Add(new() { name = "Brian", ping = UnityEngine.Random.Range(998, 999), zone = SceneManager.GetActiveScene().name });
			return true;
		}
	}
